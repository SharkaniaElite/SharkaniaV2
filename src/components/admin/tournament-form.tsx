...
export function TournamentForm({
    ...
    if (isOpen) {
      if (tournament) {
        setForm({
          ...
          start_datetime: tournament.start_datetime ? toLocalDatetimeInputValue(tournament.start_datetime) : "",
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      ...
      guaranteed_prize: form.guaranteed_prize ? Number(form.guaranteed_prize) : null,
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" value={form.buy_in} onChange={(e) => update("buy_in", e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>GTD Prize</label>
            <input type="number" value={form.guaranteed_prize} onChange={(e) => update("guaranteed_prize", e.target.value)} className={inputClass} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" value={form.late_registration_minutes} onChange={(e) => update("late_registration_minutes", e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Max Jugadores</label>
            <input type="number" value={form.max_players} onChange={(e) => update("max_players", e.target.value)} className={inputClass} placeholder="Ilimitado" />
          </div>
        </div>

        <div>
          <label className={labelClass}>Descripción</label>
          <textarea value={form.description} onChange={(e) => update("description", e.target.value)} className={`${inputClass} min-h-[60px] resize-y`} />
        </div>
