      ...
      >
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          {!isLoading && isAuthenticated && isAdmin && (
            ...
            >
              <Settings size={14} className={isActive("/admin") ? "" : "group-hover:rotate-90"} />
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-sk-bg-2 border border-sk-border-2 rounded-lg shadow-sk-xl overflow-hidden z-[50]">
                    <div className="px-4 py-3 border-b border-sk-border-2">
                      <p className="text-sk-sm font-semibold text-sk-text-1 truncate">{profile.display_name ?? "Usuario"}</p>
...
>
      ...
      {/* Mobile search */}
      {searchOpen && (
        <div className="fixed top-14 left-0 right-0 z-[201] p-4 bg-sk-bg-1 border-b border-sk-border-2 md:hidden">
          <GlobalSearch variant="full" />
        </div>
      )}
        ...
        <div style={MENU_BG} className="lg:hidden">
          ...
          {NAV_LINKS.map((link) => (
            <Link
              ...
              to={link.href}
              className={cn(
                "text-sk-md font-medium px-4 py-3 rounded-sm w-full transition-colors",
                isActive(link.href) ? "text-sk-text-1 bg-white/[0.05]" : "text-sk-text-2 hover:text-sk-text-1"
              )}
            >
              {link.label}
            </Link>
