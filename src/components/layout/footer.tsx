// src/components/layout/footer.tsx
import { Link } from "react-router-dom";

// Agregamos Contacto como un enlace externo (isExternal)
const FOOTER_LINKS = [
  { label: "Ranking", href: "/ranking" },
  { label: "Sistema ELO", href: "/sistema-elo" },
  { label: "Calendario", href: "/calendar" },
  { label: "Clubes", href: "/clubs" },
  { label: "Ligas", href: "/leagues" },
  { label: "Comparador", href: "/compare" },
  { label: "Blog", href: "/blog" },
  { label: "Glosario", href: "/glosario" },
  { label: "Contacto", href: "mailto:contacto@sharkania.com", isExternal: true },
];

const LEGAL_LINKS = [
  { label: "Términos y Condiciones", href: "/terms" },
  { label: "Política de Privacidad", href: "/privacy" },
];

export function Footer() {
  return (
    <footer className="py-8 border-t border-sk-border-1 bg-sk-bg-0">
      <div className="w-full max-w-[1200px] mx-auto px-6">
        {/* Top row */}
        <div className="flex justify-between items-center flex-wrap gap-4 max-sm:flex-col max-sm:text-center">
          <div>
            <div className="flex items-center gap-2 mb-2 max-sm:justify-center">
              <svg width="20" height="20" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7,36 L18,4 L38,24 L33,36 Z" fill="#22d3ee"/>
              </svg>
              <span className="font-extrabold text-sk-text-1 text-sk-md tracking-tight">
                Sharkania
              </span>
            </div>
            <p className="text-sk-xs text-sk-text-2">
              © {new Date().getFullYear()} Sharkania. La plataforma global de poker competitivo.
            </p>
          </div>
          
          <div className="flex flex-col items-end gap-3 max-sm:items-center">
            {/* Navegación Principal */}
            <div className="flex gap-5 flex-wrap justify-end max-sm:justify-center">
              {FOOTER_LINKS.map((link) => (
                link.isExternal ? (
                  <a
                    key={link.label}
                    href={link.href}
                    className="text-sk-xs text-sk-text-2 hover:text-sk-text-1 transition-colors duration-sk-fast"
                  >
                    {link.label}
                  </a>
                ) : (
                  <Link
                    key={link.label}
                    to={link.href}
                    className="text-sk-xs text-sk-text-2 hover:text-sk-text-1 transition-colors duration-sk-fast"
                  >
                    {link.label}
                  </Link>
                )
              ))}
            </div>

            {/* Navegación Legal & 18+ */}
            <div className="flex items-center gap-4 flex-wrap justify-end max-sm:justify-center">
              <div className="flex gap-4 border-r border-sk-border-2 pr-4 max-sm:border-r-0 max-sm:pr-0">
                {LEGAL_LINKS.map((link) => (
                  <Link
                    key={link.label}
                    to={link.href}
                    className="text-[11px] text-sk-text-2 hover:text-sk-text-1 transition-colors duration-sk-fast"
                  >
                    {link.label}
                  </Link>
                ))}
              </div>
              <div className="inline-flex items-center gap-1.5 px-1.5 py-0.5 bg-sk-bg-2 border border-sk-border-2 rounded text-[10px] font-mono text-sk-text-2 select-none">
                <span className="w-1.5 h-1.5 rounded-full bg-sk-red"></span>
                18+ ONLY
              </div>
            </div>
          </div>
        </div>

        {/* Divider */}
        <div className="w-full h-px bg-sk-border-1 my-4" />

        {/* Bottom row */}
        <div className="flex justify-between items-center flex-wrap gap-4 max-sm:flex-col max-sm:text-center">
          <p className="text-[11px] text-sk-text-2">
            Realizado por{" "}
            <a
              className="text-sk-accent underline decoration-sk-accent/30 hover:decoration-sk-accent"
            >
              clerigopoker
            </a>
            {" "}y{" "}
            <a
              href="https://brujulamarketing.cl"
              target="_blank"
              rel="noopener noreferrer"
              className="text-sk-accent hover:underline"
            >
              brújulamarketing.cl
            </a>
            {" "}para la comunidad global de póker
          </p>

          {/* Contacto Directo y WhatsApp */}
          <div className="flex items-center gap-5">
            {/* Nuevo botón de Email */}
            <a
              href="mailto:contacto@sharkania.com"
              className="flex items-center gap-1.5 text-[11px] text-sk-text-2 hover:text-white transition-colors duration-sk-fast"
              aria-label="Envíanos un correo electrónico"
            >
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
              </svg>
              contacto@sharkania.com
            </a>

            <a
              href="https://chat.whatsapp.com/C2K1IabbgI8BEwIcLdinw7"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-[11px] text-sk-text-2 hover:text-sk-green transition-colors duration-sk-fast"
              aria-label="Únete a nuestra comunidad de WhatsApp"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Comunidad WhatsApp
            </a>

            <a
              href="https://www.instagram.com/sharkania_oficial/"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-[11px] text-sk-text-2 hover:text-[#E1306C] transition-colors duration-sk-fast"
              aria-label="Síguenos en Instagram"
            >
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect width="20" height="20" x="2" y="2" rx="5" ry="5"/>
                <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/>
                <line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/>
              </svg>
              Instagram
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}