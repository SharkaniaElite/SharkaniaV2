...
export function IgnitionChampionshipPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // Si la persona ya tiene esos datos pero no ha iniciado sesión, p_user_id será null, por lo que detectará si ALGÚN usuario lo tiene.
    const { data: isDuplicate, error: rpcError } = await supabase.rpc("check_ignition_duplicate", {
      p_email: isExistingUser ? null : emailToLink, // Si es login, no evaluamos su email de Sharkania contra el de Ignition
    ...
    if (isExistingUser) {
      // ==========================================
      // FLUJO 1: LOGIN (Ya tiene cuenta en Sharkania)
      // ==========================================
      ...
      const { data: doubleCheckDuplicate } = await supabase.rpc("check_ignition_duplicate", {
        p_email: emailToLink, // Ahora sí usamos el email del form para Ignition
        p_nickname: nickToLink,
      ...
      if (doubleCheckDuplicate) {
        ...
        await supabase.auth.signOut(); // Deslogueamos por seguridad
        setLoading(false);
        return;
      }
      ...
      // ==========================================
      // FLUJO 2: SIGN UP (Nuevo en Sharkania)
      // ==========================================
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white relative overflow-hidden font-sans">
      ...
      <div className="absolute top-0 inset-x-0 p-6 z-20 flex justify-between items-center max-w-7xl mx-auto">
        <Link to="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <span className="text-2xl">🦈</span>
          <span className="text-xl font-black tracking-tight text-white">Sharkania</span>
        </Link>
        <Link to="/login" className="text-sm font-semibold text-orange-400 hover:text-orange-300">
          Login Standard →
        </Link>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-black/50 border border-white/5 rounded-xl p-4 backdrop-blur-sm">
              <div className="text-orange-500 mb-2"><Trophy size={24} /></div>
              <p className="text-xs font-mono text-gray-500 uppercase mb-1">Pozo Garantizado por fecha</p>
          ...
          <div className="space-y-6 pt-4 border-t border-white/10 mt-8">
            ...
            <div className="flex gap-4 items-start">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">Crea tu cuenta en la sala si aún no la tienes utilizando nuestro enlace oficial. Cuando hagas tu primer depósito, obtendrás una bonificación del 100% hasta $1000 USD.</p>
                <a href="https://record.revenuenetwork.com/_s_OAdmC6KUepsRaI0hkgHmNd7ZgqdRLk/1/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-orange-400 hover:text-orange-300 mt-2 font-semibold bg-orange-500/10 px-3 py-1.5 rounded border border-orange-500/20">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 sm:p-8 relative shadow-2xl">
            ...
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {/* CAMPOS COMUNES */}
                <div>
                  <label className={labelClass}>Correo Electrónico * (El mismo que ocupaste para registrarte en Ignition Póker)</label>
                  <input
                    type="email"
                    value={email}
                ...
                {/* 🔥 EL CAPTCHA AHORA ESTÁ FUERA DE LA CONDICIÓN Y SE RENDERIZA SIEMPRE */}
                ...
                <Button
                  ...
                  isLoading={loading}
                  disabled={!captchaToken || loading} // 🔥 Ahora ambos flujos requieren el token
                >
